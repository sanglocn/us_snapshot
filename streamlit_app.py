...
def render_dashboard(df_etf: pd.DataFrame, df_rs: pd.DataFrame) -> None:
    ...
    # Prepare data: add date_str, sort by code
    df_heat_latest = df_heat_latest.copy()
    df_heat_latest['date_str'] = df_heat_latest['date'].dt.strftime("%Y-%m-%d")
    df_heat_latest = df_heat_latest.sort_values('code').reset_index(drop=True)
    ...
    fig = px.scatter(
        ...
        y='PriceFactor',
        color='code',
        custom_data=['date_str', 'ticker', 'code', 'PriceFactor', 'VolumeFactor'],
        height=550,
    )
    fig.update_traces(
        ...
        hovertemplate=(
            ...
            "<i>%{customdata[0]}</i><br>"      # date_str
            "Code: %{customdata[2]}<br>"       # code
    ...
    # Get trace sizes for global index calculation
    sizes = [len(trace.x) for trace in fig.data]
    ...
    selected_from_events = []
    if events:
        for ev in events:
            if 'points' in ev:
                for p in ev['points']:
                    cn = p.get('curveNumber', 0)
                    pn = p.get('pointNumber')
                    if cn is not None and pn is not None:
                        global_idx = sum(sizes[:int(cn)]) + int(pn)
                        if 0 <= global_idx < len(df_heat_latest):
                            selected_from_events.append(df_heat_latest.iloc[global_idx]['ticker'])
            else:
                # Click event
                cd = ev.get("customdata")
                if cd and isinstance(cd, (list, tuple)) and len(cd) >= 2:
                    selected_from_events.append(cd[1])
                else:
    ...
    if selected_from_events:
        st.session_state.selected_tickers = list(set(selected_from_events))
    ...
    with col_b:
        if selected_tickers:
            st.info(f"Filtering heatmaps to: {', '.join(sorted(selected_tickers))}")
        else:
    ...
    code_series = df_for_heat.groupby('ticker')['code'].first()
    order_df = code_series.reset_index().sort_values(['code', 'ticker'])
    ticker_list = order_df['ticker'].tolist()
    code_map = code_series.to_dict()
    ...
    vol_customdata = [[code_map.get(t, "")] * len(dates_sorted) for t in ticker_list]
    price_customdata = [[code_map.get(t, "")] * len(dates_sorted) for t in ticker_list]
    ...
    with col1:
        fig_vol = go.Figure(
            data=go.Heatmap(
                ...
                y=ticker_list,
                colorscale='Viridis',
                colorbar=dict(title='Volume Factor'),
                hovertemplate=(
                    "Ticker: %{y}<br>"
                    "Date: %{x}<br>"
                    "Volume Factor: %{z:.4f}<br>"
    ...
    with col2:
        fig_price = go.Figure(
            data=go.Heatmap(
                ...
                y=ticker_list,
                colorscale='Plasma',
                colorbar=dict(title='Price Factor'),
                hovertemplate=(
                    "Ticker: %{y}<br>"
                    "Date: %{x}<br>"
                    "Price Factor: %{z:.4f}<br>"
                    "Code: %{customdata}<extra></extra>"
                ),
